...
const Playground: React.FC<PlaygroundProps> = ({problem, setSuccess, setSolved}) => {
  ...
  const handleSubmit = async () => {
    ...
    try{
      userCode = userCode = userCode.slice(userCode.indexOf(problem.starterFunctionName))
          ...
          <div className="flex">
            {problem.examples.map((example,index)=>(
              ...
              <div className="flex flex-wrap items-center gap-y-4">
                <div className={`font-medium items-center transition-all focus:outline-none inline-flex bg-dark-fill-3 gover:bg-dark-fill-2 relative rounded-lg px-4 py-1 cursor-pointer whitespace-nowrap ${activeTestCaseId === index ? "text-white" : "text-gray-500"}`}>
                  Case {index+1}
                </div>
              </div>
            </div>
