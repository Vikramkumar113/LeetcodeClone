...
const Signup:React.FC<SignupProps> = () => {
  ...
  const handleRegister = async (e:React.FormEvent<HTMLFormElement>) =>{
    e.preventDefault();
    if(!inputs.email || !inputs.displayName || !inputs.password) return toast.success("Please all the fields", {position:"top-center", autoClose:3000, theme:'dark'});
    try{
  ...
  <div>
          <label htmlFor="displayName" className='text-sm font-medium block mb-2 text-gray-300'>Dispaly Name</label>
          <input onChange={handleChangeInput} type="text" name="displayName" className='border-2 outline-none sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 bg-gray-600 border-gray-500 placeholder-gray-400 text-white' placeholder='Vikram Jangid' />
        </div>
  <div>
